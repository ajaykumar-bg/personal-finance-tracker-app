...
export const StatsCard: React.FC<StatsCardProps> = ({ stats }) => {
	...
	return (
		<Card style={styles.card}>
			<Card.Content>
				<View style={isMobile ? styles.containerColumn : styles.container}>
					{stats.map((stat, index) => (
						<View
							key={index}
							style={[
								styles.stat,
								isMobile && styles.statColumn,
								isMobile &&
									index < stats.length - 1 &&
									styles.statColumnDivider,
							]}
						>
							<Text
								variant={isMobile ? 'bodyMedium' : 'bodySmall'}
								style={[styles.label, isMobile && styles.labelColumn]}
								numberOfLines={1}
							>
								{stat.label}
							</Text>
